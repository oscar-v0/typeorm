...
export class EntityMetadataBuilder {
    ...
    protected createEntityMetadata(
        ...
        const inheritanceTree: any[] =
            tableArgs.target instanceof Function
    ...
    protected createEmbeddedsRecursively(
        ...
        return embeddedArgs.map((embeddedArgs) => {
            ...
            if (typeof embeddedMetadata.type === "function") {
                ...
                const embeddedMetadataArgsStorage =
                    new EntitySchemaTransformer().transform([
                        embeddedMetadata.type,
                    ]);

                embeddedMetadata.columns = embeddedMetadataArgsStorage
                    .filterColumns(embeddedMetadata.type.options.name)
                    .map((args) => {
                        return new ColumnMetadata({
                            connection: this.connection,
                            entityMetadata,
                ...
                embeddedMetadata.relations = embeddedMetadataArgsStorage
                    .filterRelations(embeddedMetadata.type.options.name)
                    .map((args) => {
                        return new RelationMetadata({
                            entityMetadata,
                            embeddedMetadata,
                ...
                embeddedMetadata.listeners = embeddedMetadataArgsStorage
                    .filterListeners(embeddedMetadata.type.options.name)
                    .map((args) => {
                        return new EntityListenerMetadata({
                            entityMetadata,
                            embeddedMetadata,
                ...
                embeddedMetadata.indices = embeddedMetadataArgsStorage
                    .filterIndices(embeddedMetadata.type.options.name)
                    .map((args) => {
                        return new IndexMetadata({
                            entityMetadata,
                            embeddedMetadata,
                ...
                embeddedMetadata.uniques = embeddedMetadataArgsStorage
                    .filterUniques(embeddedMetadata.type.options.name)
                    .map((args) => {
                        return new UniqueMetadata({
                            entityMetadata,
                            embeddedMetadata,
                ...
                embeddedMetadata.relationIds = embeddedMetadataArgsStorage
                    .filterRelationIds(embeddedMetadata.type.options.name)
                    .map((args) => {
                        return new RelationIdMetadata({ entityMetadata, args });
                    });
                embeddedMetadata.relationCounts = embeddedMetadataArgsStorage
                    .filterRelationCounts(embeddedMetadata.type.options.name)
                    .map((args) => {
                        return new RelationCountMetadata({
                            entityMetadata,
                            args,
                ...
                embeddedMetadata.embeddeds = this.createEmbeddedsRecursively(
                    entityMetadata,
                    embeddedMetadataArgsStorage.filterEmbeddeds(
                        embeddedMetadata.type.options.name
                    )
                );
            }
